...
public class UserIDInfoDAO {
	...
	public UserIDResponse createUserInfo(UserID userID) {
		...
		try {
			...
			userId.setUser_id("PATID1239");
			final HttpEntity<UserID> request = new HttpEntity<>(userId);
			final ResponseEntity<UserIDResponse> response = restTemplate.exchange(
					"http://18.188.253.202:3000/api/org.orcicare.network.business.CreateOrciMember", HttpMethod.POST,
					request, UserIDResponse.class);
